...
def update_emerging_issues_data(collection):
    ...
    if updates:
        try:
            ...
            logging.error("Bulk write error: %s", e.details)        except Exception as e:
...
### Key Changes:
1. **Conversion to Python float:** In the `perform_sentiment_analysis` function, `numpy.float32` values are converted to Python floats before they are added to the result dictionary. This ensures compatibility with MongoDB.