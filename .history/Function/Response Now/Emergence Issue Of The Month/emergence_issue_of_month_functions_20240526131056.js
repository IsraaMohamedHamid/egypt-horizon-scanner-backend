      ...
      if (code !== 0) {
        ...
        try {
          ...
          results.forEach(async result => {
            await collection.updateOne({ link: result.link }, { $set: result }, { upsert: true });
          });
          resolve(results);
        } catch (error) {
...
async function compileIssueData(issue, issueDocuments) {
  ...
  const aggregation = await EmergenceIssueOfTheMonthDataModel.aggregate([
    ...
    { $group: { _id: "$emergingIssue", sources: { $addToSet: "$source" }, sdgTargeted: { $addToSet: "$sdgTargeted" } } }
  ]);
  ...
  const sdgAggregation = await EmergenceIssueOfTheMonthDataModel.aggregate([
    { $match: { emergingIssue: issue } },
    { $unwind: "$sdgTargeted" },
    { $group: { _id: "$sdgTargeted", count: { $sum: 1 } } }
  ]);
...
async function updateEmergenceIssue(issue, data) {
  const { sources, sdgTargeted, sdgTargetedDictionary, totalDataCount, positiveSentimentAnalysisDataCount, neutralSentimentAnalysisDataCount, negativeSentimentAnalysisDataCount } = data;
  await EmergenceIssueOfTheMonthModel.updateOne(
    { emergingIssue: issue },
    { $set: { sources, sdgTargeted, sdgTargetedDictionary, totalDataCount, positiveSentimentAnalysisDataCount, neutralSentimentAnalysisDataCount, negativeSentimentAnalysisDataCount, ...data } },
    { upsert: true }
  );
