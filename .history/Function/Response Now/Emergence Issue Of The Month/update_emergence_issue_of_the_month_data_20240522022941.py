...
def get_mongo_connection():
    mongo_uri = 'mongodb+srv://doadmin:w94yB2Y17dWE8C63@dbaas-db-5626135-310aba91.mongo.ondigitalocean.com/egypt-horizon-scanner'
...
def sentiment_analysis(text, model, tokenizer):
    ...
    return model.config.id2label[predicted_class_index], probabilities.numpy().tolist()

def update_emerging_issues_data2(data, model, tokenizer):
...
# Update data in MongoDB using bulk operations
def update_emerging_issues_data():
    ...
    for index, row in data.iterrows():
        ...
        # print(f"Processed record {index + 1}: {sentiment_data}")
        update_dict = {
            'filter': {'_id': row['_id']},
            'update': {'$set': {'sentiment': sentiment_data}}
        }
    ...
    if updates:
        print(collection.bulk_write([UpdateOne(x['filter'], x['update']) for x in updates]))
        collection.bulk_write([UpdateOne(x['filter'], x['update']) for x in updates])
        print(f"Updated {len(updates)} records.")


if __name__ == "__main__":
    try:
        update_emerging_issues_data()
