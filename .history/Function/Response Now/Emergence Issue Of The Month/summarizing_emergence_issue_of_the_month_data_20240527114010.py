...
def perform_sentiment_analysis(text):
    ...
    predicted_class, probabilities = sentiment_analysis(text, new_model, new_tokenizer)
    sentiment_scores = {new_model.config.id2label[i]: prob for i, prob in enumerate(probabilities[0])}
    max_sentiment = max(sentiment_scores, key=sentiment_scores.get)
    weight = {'positive': 1.0, 'neutral': 0.75, 'negative': 0.5}.get(getSentimentLabel(sentiment), 0.75) 
    return lang, sentiment_scores, max_sentiment, sentiment_scores[max_sentiment], weight
...
def analyze_description_sentiment(description, model, tokenizer):
    ...
    return {'sentimentAnalysis': sentiment_label, 'weight': weight, 'score': score, 'language': lang}
...
def process_url(item, collection):
    try:
        ...
        lang, sentiment_scores, sentiment, score, weight = perform_sentiment_analysis(summary)
