...
def update_emerging_issues_data():
    ...
    data['sentiment'] = data['description'].apply(lambda x: analyze_description_sentiment(x, model, tokenizer))

    bulk = collection.initialize_ordered_bulk_op()
    
    for index, row in data.iterrows():
        bulk.find({'_id': row['_id']}).update({'$set': {'sentiment': row['sentiment']}})
    bulk.execute()
