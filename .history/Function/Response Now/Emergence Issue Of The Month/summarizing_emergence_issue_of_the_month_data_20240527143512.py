...
def process_url(item, model, tokenizer):
    try:
        url = item.get('link')
        if not url:
        ...
        print(summary)
        lang, sentiment, score, weight = perform_sentiment_analysis(summary, model, tokenizer)
        print(sentiment)
...
def update_emerging_issues_data(collection):
    ...
    for index, row in data.iterrows():
        result = process_url(row.to_dict(), model, tokenizer)
        update_dict = {
            'filter': {
                '_id': row['_id'],
                'link': row['link'],
