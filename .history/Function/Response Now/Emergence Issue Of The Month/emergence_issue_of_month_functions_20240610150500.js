...
export const emergingIssueComponentsCalculation = async () => {
  try {
    ...
    const emergingIssues = uniqueIssueTitles.map(issueTitle => {
      ...
      const pillars = [...new Set(issueDocuments.flatMap(doc => doc.pillar))];
      const dimensions = [...new Set(issueDocuments.flatMap(doc => doc.dimension))];
    ...
    for (const issue of emergingIssues) {
      ...
      updateData.indicators = issue.indicators.join(', '); // Convert array to comma-separated string
