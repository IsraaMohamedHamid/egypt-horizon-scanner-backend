...
1. **Removed Unused Imports**: Eliminated unnecessary imports to speed up the script loading time.
2. **OpenAI API Optimization**: Combined classification and information extraction into a single API call.
3. **Concurrent Processing**: Ensured that async tasks are running concurrently to utilize asyncio effectively.
4. **Batch MongoDB Updates**: Optimized the database update operations by using bulk updates.

These changes will help reduce the execution time of the code while maintaining its functionality.
