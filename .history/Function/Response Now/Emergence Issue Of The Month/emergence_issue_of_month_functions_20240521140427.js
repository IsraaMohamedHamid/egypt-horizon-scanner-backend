...
export async function watchEmergingIssues3() {
  try {
    await client.connect();
    console.log("Connected successfully to server");
...
export async function watchEmergingIssues() {
  try {
    ...
    changeStream.on('change', async (change) => {
      console.log('Change detected:', change);
      unprocessedDocuments.push(change.fullDocument._id);

      // Optionally, process in batches or on a timer
      // For example, if (unprocessedDocuments.length > 10) { ... }
    });
      ...
      if (unprocessedDocuments.length > 0) {
        try {
          ...
          await collection.updateMany({
            _id: { $in: unprocessedDocuments }
          }, {
            $set: { processed: true }
          });
          console.log('Batch processed and documents marked as processed');
          unprocessedDocuments = [];  // Reset the array after processing
        } catch (processError) {
          console.error('Error processing batch change:', processError);
        }
      }
    }, 60000);  // Adjust time as necessary, e.g., every 60 seconds
