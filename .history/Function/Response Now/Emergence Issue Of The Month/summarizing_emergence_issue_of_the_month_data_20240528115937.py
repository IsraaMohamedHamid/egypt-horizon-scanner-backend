...
summarizer = pipeline("summarization", model=model, tokenizer=summary_tokenizer)

# Load the model and tokenizer for the new sentiment analysis
senti_model_name = "sakasa007/finetuning-sentiment-text-mining" #"jysh1023/tiny-bert-sst2-distilled"
...
def perform_sentiment_analysis(description,  sentiModel, sentiTokenizer):
    
    lang = detect_language(description)
    
    inputs = sentiTokenizer(description, return_tensors="tf", truncation=True, padding=True, max_length=512)
    ...
    sentiment_label = sentiModel.config.id2label[predicted_class_index]
    
    weight = {'positive': 1.0, 'neutral': 0.75, 'negative': 0.5}.get(sentiment_label, 0.75)
   
    score = probabilities.numpy()[0][predicted_class_index].item()
    
    return lang, sentiment_label, score, weight
...
def process_url(item, sentiModel, sentiTokenizer):
    try:
        url = item.get('link')
        if not url:
...
def update_emerging_issues_data(collection, sentiModel, sentiTokenizer):
    ...
    for index, row in data.iterrows():
        result = process_url(row.to_dict(), sentiModel, sentiTokenizer)
        update_dict = {
            'filter': {
                '_id': row['_id'],
                'link': row['link'],
...
if __name__ == "__main__":
    try:
        ...
        update_emerging_issues_data(db_collection, senti_model, senti_tokenizer)
    except Exception as e:
