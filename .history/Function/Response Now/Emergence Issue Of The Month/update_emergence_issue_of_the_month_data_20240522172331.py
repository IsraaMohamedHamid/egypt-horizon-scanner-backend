...
def update_emerging_issues_data(collection, model, tokenizer):
    ...
    for index, row in data.iterrows():
        ...
        update_dict = {'filter': {'_id': row['_id'], 'issueTitle': row['issueTitle'], 'emergingIssue': row["emergingIssue"], "sdgTargeted" 'description': row['description']}, 'update': {'$set': sentiment_data}}
        print(f"Processed record {index + 1}: {update_dict['filter']['_id']}")
