...
import { EmergenceIssueOfTheMonthDataModel } from './models';
import { determinePriority, compileIssueData, updateEmergenceIssue } from './utils';
...
const compileIssueData = async (issueTitle, issueDocuments) => {
  // Compile the data for the issue based on your application logic
  // Example: Summarize data from issueDocuments or fetch additional data
  return {
    issueTitle,
...
const updateEmergenceIssue = async (issueTitle, updateData) => {
  await EmergenceIssueOfTheMonthDataModel.updateMany({ issueTitle }, { $set: updateData });
};
