...
const emergingIssueComponentsCalculation = async function () {
  try {
    ...
    await EmergenceIssueOfTheMonthDataModel.find().then(async (data) => {
      const positiveDataCount = data.filter(doc => doc.sentimentAnalysis === "positive").length;
      const neutralDataCount = data.filter(doc => doc.sentimentAnalysis === "neutral").length;
      const negativeDataCount = data.filter(doc => doc.sentimentAnalysis === "negative").length;
