...
export const emergingIssueComponentsCalculation = async () => {
  try {
    ...
    const emergingIssues = uniqueIssueTitles.map(issueTitle => {
      const issueDocuments = combinedData.filter(doc => doc.uniqueIssueTitles === issueTitle);
      ...
      return { issueTitle: issueTitle, description: description, totalWeight: totalWeight, repetition: repetition };
    });
    ...
    for (const issue of emergingIssues) {
      ...
      const issueDocuments = combinedData.filter(doc => doc.Id === issue.issueTitle);
