...
def update_emerging_issues_data(collection, model, tokenizer):
    ...
    if updates:
        try:
            ...
            # Uncomment the following lines to debug and print details of each update operation
            # print(f"Collection: {collection}")
            for x in updates:
                # print("Description:", x['filter']['description'])
                # print("$set:", x['update']['$set'])
                newUpdate.append(UpdateOne({
                    'description': x["filter"]['description']
                },
                                           {
                    '$set': {
                        "sentimentAnalysis": x["update"]["$set"]["sentimentAnalysis"],
                        "weight": x["update"]["$set"]["weight"],
                        "score": x["update"]["$set"]["score"],
                        "language": x["update"]["$set"]["language"],
                    }
                },))
                
                # print("Filter:", x['filter'])
                # print("Update:", x['update'])
            result2 = collection.bulk_write([UpdateOne(x['filter'], x['update']) for x in updates], ordered=False)

            print(f"Bulk writing updates: {newUpdate}")
            result = collection.bulk_write(newUpdate)
            print(f"Successfully updated {result.modified_count} records.")
        except Exception as e:
