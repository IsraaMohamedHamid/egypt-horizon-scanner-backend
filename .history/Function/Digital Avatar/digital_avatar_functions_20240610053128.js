...
export const DidimensionsDataSummary = async (data) => {
...
export const DidimensionsDataUpdate = async () => {
  return new Promise((resolve, reject) => {
    try {
      console.log(`START: Processing emerging issues.`);
      // spawn new child process to call the python script
...
export const DidimensionsComponentsCalculation = async () => {
  try {
    console.log('START: Calculating components for emerging issues.');
    const uniqueIssues = await DimensionsModel.distinct('Didimensions');
    console.log(`Processing ${uniqueIssues.length} unique emerging issues.`);

    // Prepare data from sources (all from the model)
    let combinedData = [];
    for (const issue of uniqueIssues) {
      const issueDocuments = await DimensionsModel.find({ Didimensions: issue });
    ...
    const Didimensionss = uniqueIssueTitles.map(issueTitle => {
    ...
    for (const issue of Didimensionss) {
...
async function compileIssueData(issueTitle, issueDocuments) {
  ...
  const aggregation = await DimensionsModel.aggregate([
    { $match: { issueTitle } },
    {
      $group: {
        _id: "$Didimensions",
  ...
  const sdgAggregation = await DimensionsModel.aggregate([
    { $match: { issueTitle } },
    { $unwind: "$sdgTargeted" },
    {
      $group: {
...
async function updateEmergenceIssue(issueTitle, data) {
  ...
  await DimensionshModel.updateOne(
    { Didimensions: issueTitle },
    {
      $set: {
        sources,
        sdgTargeted,
...
export default {
  DidimensionsDataUpdate,
  DidimensionsComponentsCalculation
};