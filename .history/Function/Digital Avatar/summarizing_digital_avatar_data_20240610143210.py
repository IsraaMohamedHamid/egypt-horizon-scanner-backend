...
async def classify_and_extract_text(url):
    prompt = (
        f"From the following Link {url}, classify the text into maximum 5 themes from the 58 themes listed below and extract relevant information:\n"
        "Themes:\n"
...
# Ensure parse_classification_and_extraction correctly processes the response
def parse_classification_and_extraction(response):
    # Implement this function to parse the response from GPT and extract themes and relevant information.
    # This is a placeholder implementation and should be customized based on the actual response format.
    themes = {}
    extracted_info = {
        "Main objectives": [],
        "Main outcomes": [],
    ...
    # Example parsing logic (this needs to be adjusted based on the actual response structure)
    for line in response.split('\n'):
        if "Theme:" in line:
            current_theme = line.split(':')[1].strip()
            themes[current_theme] = []
        elif "Main objectives:" in line:
            extracted_info["Main objectives"].append(line.split(':')[1].strip())
        elif "Main outcomes:" in line:
            extracted_info["Main outcomes"].append(line.split(':')[1].strip())
        elif "Problem statement:" in line:
            extracted_info["Problem statement"].append(line.split(':')[1].strip())
        elif "KPIs:" in line:
            extracted_info["KPIs"].append(line.split(':')[1].strip())
        else:
            if current_theme:
                themes[current_theme].append(line.strip())
    
    return themes, extracted_info

# Function to parse classification and extraction response
def parse_classification_and_extraction2(response):
