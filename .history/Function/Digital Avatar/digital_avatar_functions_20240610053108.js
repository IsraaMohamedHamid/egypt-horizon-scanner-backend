...
import {
  DimensionsModel
} from '../../Model/Digital Avatar/dimension';
import {
  spawn
} from 'child_process';
...
export const DidimensionsComponentsCalculation = async () => {
  try {
    ...
    const uniqueIssues = await EmergenceIssueOfTheMonthDataModel.distinct('Didimensions');
    console.log(`Processing ${uniqueIssues.length} unique emerging issues.`);

    // Prepare data from sources (all from the model)
    let combinedData = [];
    for (const issue of uniqueIssues) {
      const issueDocuments = await EmergenceIssueOfTheMonthDataModel.find({ Didimensions: issue });
