...
const updateEmergingIssueDataByID = async (req, res, next) => {
  try {
    await EmergenceIssueOfTheMonthDataModel.findByIdAndUpdate(req.params.id, { $set: req.body });
...
const updateEmergingIssueDataByEmergingIssueName = async (req, res, next) => {
  try {
    await EmergenceIssueOfTheMonthDataModel.findOneAndUpdate({ emergingIssue: req.params.emergingIssue }, { $set: req.body });
