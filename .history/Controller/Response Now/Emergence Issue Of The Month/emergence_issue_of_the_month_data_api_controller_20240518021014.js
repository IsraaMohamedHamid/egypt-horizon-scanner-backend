...
import { find, create, findByIdAndUpdate, findById, findOneAndUpdate, findOne, findByIdAndRemove, findOneAndRemove } from '../../../Model/Response Now/Emergence Issue Of The Month/emergence_issue_of_the_month_data_model.js';
...
const getEmergingIssuesData = async (req, res, next) => {
  try {
    const issues = await find({}).sort(sortCriteria);
    res.send(issues);
  } catch (error) {
    next(error);
  }
...
const getEmergingIssueDataByName = async (req, res, next) => {
  try {
    const issue = await find({ emergingIssue: req.params.emergingIssue }).sort(sortCriteria);
    res.send(issue);
  } catch (error) {
    next(error);
  }
