  ...
  try {
    ...
    const tasks = municipal_divisions.map(async (municipal_division) => {
      const projects = await projectsModel.find({
        Governorate_Name_EN: municipal_division.Governorate_Name_EN
      });
      ...
      if (projects.length > 0) {
        ...
        console.log(`${municipal_division.Governorate_Name_EN}: map: ${Object.values(map)}, Contains zero: ${Object.values(map).includes(0)}, no Projects: ${noProjects}, least Projects: ${leastProjects}, most Projects: ${mostProjects}`);
