...
import { DistrictSchema, districtModel } from '../../../Model/Response Now/Interventions/district_model.js';
...
export async function countProjectsByTheme(districtName) {
...
export async function updateDistrictData(districtName, themeCounts) {
...
export const createDistrict = async (req, res) => {
  try {
    const newDivision = await districtModel.create(req.body);
...
export const updateDistrictByID = async (req, res) => {
  try {
    const updatedDivision = await districtModel.findByIdAndUpdate(
      req.params.id,
      { $set: req.body },
      { new: true }
    );
...
export const updateDistrictByDistrictNameEN = async (req, res) => {
  try {
    const updatedDivision = await districtModel.findOneAndUpdate(
...
export const deleteDistrictByID = async (req, res) => {
  try {
    const deletedDivision = await districtModel.findByIdAndRemove(req.params.id);
...
export const deleteDistrictByDistrictNameEN = async (req, res) => {
  try {
    const deletedDivision = await districtModel.findOneAndRemove({
      Municipal_Division_Name_EN: req.params.districtNameEN
    });
    res.send(deletedDivision);
  } catch (error) {
