const {
    statesModel, 
    GovernorateSchema
} from '../../../Model/Response Now/Interventions/state_model.js');
...
statesModel.find().then(function (state) {
    for (let i = 0; i < state.length; i++) {
      ...
      // Get all data
      projectsModel.find({
        Governorate_Name_EN: state[i].Governorate_Name_EN
            ...
            try {
              ...
              map['R_C'] = await projectsModel.countDocuments({
                $and: [{Governorate_Name_EN: {
                  $eq: state[i].Governorate_Name_EN
                }},
                {theme: {
                  $in: ['R_C']
                }}]
...
const updateGovernorateByGovernorateNameEN = ((req, res, next) => {
    //to access :id ---> req.params.id
    statesModel.findOneAndUpdate({ Governorate_Name_EN: req.params.stateNameEN }, {$set:req.body}).then(function () {
        statesModel.findOne({ Governorate_Name_EN: req.params.stateNameEN }).then(function (state) {
            res.send(state);
        });
    });
})
...
const deleteGovernorateByGovernorateNameEN = ((req, res, next) => {
    //to access :id ---> req.params.id
    statesModel.findOneAndRemove({ Governorate_Name_EN: req.params.stateNameEN }).then(function (state) {
        res.send(state);
    });
})
