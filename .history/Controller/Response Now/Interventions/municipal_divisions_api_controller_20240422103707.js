...
const getMunicipalDivisions = ((req, res, next) => {
  ...
  municipalDivisionsModel.find({}, {
   ...
   for (let i = 0; i < municipal_division.length; i++) {
     ...
     // Get all data
     projectsModel.find({
       Municipal_Division_Name_EN: municipal_division[i].Municipal_Division_Name_EN
           ...
           try {
             ...
             map['R_C'] = await projectsModel.countDocuments({
               $and: [{Municipal_Division_Name_EN: {
                 $eq: municipal_division[i].Municipal_Division_Name_EN
               }},
               {theme: {
                 $in: ['R_C']
               }}]
 ...
 // Get all data
 municipalDivisionsModel.find({}).then(function (municipal_divisions) {
