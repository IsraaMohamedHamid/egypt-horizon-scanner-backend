...
const getGovernorates = ((req, res, next) => {
  ...
  governoratesModel.find({}, {
   Governorate_Name_EN: 1,
   _id: 0
 }).then(function (municipal_division) {
   for (let i = 0; i < municipal_division.length; i++) {
     let map = new Map();
     // Get all data
     projectsModel.find({
       Governorate_Name_EN: municipal_division[i].Governorate_Name_EN
     }).then(function (municipal_divisions) {
       if (municipal_divisions.length > 0) {
           ...
           try {
             ...
             map['R_C'] = await projectsModel.countDocuments({
               $and: [{Governorate_Name_EN: {
                 $eq: municipal_division[i].Governorate_Name_EN
               }},
               {theme: {
                 $in: ['R_C']
               }}]
 ...
 // Get all data
 governoratesModel.find({}).then(function (municipal_divisions) {
