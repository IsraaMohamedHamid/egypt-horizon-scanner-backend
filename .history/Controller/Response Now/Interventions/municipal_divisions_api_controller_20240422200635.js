...
const getMunicipalDivisions = async (req, res, next) => {
  try {
    const municipal_divisions = await municipalDivisionsModel.find();

    const tasks = municipal_divisions.map(async (municipal_division) => {
      console.log(`${municipal_division.Municipal_Division_Name_EN}`);

      const projects = await projectsModel.find({
        Municipal_Division_Name_EN: municipal_division.Municipal_Division_Name_EN
      });
      ...
      if (projects.length > 0) {
        ...
        // Filter out themes with the most and least projects
        ...
        // Identify themes with no projects
        const noProjects = themes.filter(theme => !themeCounts[theme] || themeCounts[theme] === 0);

        console.log(`${municipal_division.Municipal_Division_Name_EN}: Most Projects: ${mostProjects}, Least Projects: ${leastProjects}, No Projects: ${noProjects}`);
