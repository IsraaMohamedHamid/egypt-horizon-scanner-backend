...
const getMunicipalDivisions = async (req, res, next) => {
  try {
    const governorates = await municipalDivisionsModel.find();
    ...
    const tasks = governorates.map(async (governorate) => {
      ...
      if (projects.length > 0) {
        ...
        console.log(`${governorate.Governorate_Name_EN}: map: ${Object.values(map)}, Contains zero: ${Object.values(map).includes(0)}, no Projects: ${noProjects}, least Projects: ${leastProjects}, most Projects: ${mostProjects}`);
