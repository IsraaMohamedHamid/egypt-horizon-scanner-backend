export const NodeCache from 'node-cache');
export const cron from 'node-cron');
export const myCache = new NodeCache({ stdTTL: 21600 }); // Cache with a TTL of 6 hours
...
export async function countProjectsByTheme(governorateName) {
  export const themes = ['R_C', 'E_E', 'D_E', 'I_D', 'H_D', 'P_S'];
  export const themeCounts = {};
  for (export const theme of themes) {
...
export async function updateGovernorateData(governorateName, themeCounts) {
  export const lastData = myCache.get(governorateName);
  export const currentHash = hashThemeCounts(themeCounts);

  if (!lastData || lastData.hash !== currentHash || Date.now() > lastData.nextUpdateTime) {
    export const sortedThemes = Object.entries(themeCounts).sort(([, a], [, b]) => b - a);
    export const themesWithProjects = sortedThemes.filter(([, value]) => value > 0);
...
cron.schedule('0 */6 * * *', async () => {
  ...
  export const governorates = await governoratesModel.find();
  for (export const division of governorates) {
    export const themeCounts = await countProjectsByTheme(division.Municipal_Division_Name_EN);
...
export const createGovernorate = async (req, res) => {
  try {
    export const newDivision = await governoratesModel.create(req.body);
...
export const updateGovernorateByID = async (req, res) => {
  try {
    export const updatedDivision = await governoratesModel.findByIdAndUpdate(
      req.params.id,
      { $set: req.body },
      { new: true }
    );
...
export const updateGovernorateByGovernorateNameEN = async (req, res) => {
  try {
    export const updatedDivision = await governoratesModel.findOneAndUpdate(
...
export const deleteGovernorateByID = async (req, res) => {
  try {
    export const deletedDivision = await governoratesModel.findByIdAndRemove(req.params.id);
...
export const deleteGovernorateByGovernorateNameEN = async (req, res) => {
  try {
    export const deletedDivision = await governoratesModel.findOneAndRemove({
      Municipal_Division_Name_EN: req.params.governorateNameEN
    });
    res.send(deletedDivision);
  } catch (error) {
