  ...
  try {
    ...
    const tasks = governorates.map(async (governorate) => {
      ...
      if (projects.length > 0) {
        ...
        // Sorting the theme counts by count value descending
        const sortableCounts = Object.entries(themeCounts).sort(([, a], [, b]) => b - a);
        const maxCount = sortableCounts[0][1];

        // Correctly calculating the minimum count that is greater than zero
        const filteredCounts = sortableCounts.filter(([, value]) => value > 0);
        const minCount = filteredCounts.length > 0 ? filteredCounts[filteredCounts.length - 1][1] : 0;

        const mostProjects = sortableCounts.filter(([ , value]) => value === maxCount).map(([key]) => key);
        const leastProjects = filteredCounts.filter(([ , value]) => value === minCount).map(([key]) => key);
        const noProjects = themes.filter(theme => !(themeCounts[theme] > 0));
