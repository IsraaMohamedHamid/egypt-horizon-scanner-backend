...
async function countProjects(cityName) {
  ...
  for (const theme of themes) {
    counts[theme] = await projectsModel.countDocuments({
...
// Function to update city intervention details
async function updateCityInterventionType(cityName, counts) {
  const sortedThemes = Object.keys(counts).sort((a, b) => counts[b] - counts[a]);
  const noProjects = sortedThemes.find(theme => counts[theme] === 0) || '';
  const leastProjects = sortedThemes[sortedThemes.length - 1];
  const mostProjects = sortedThemes[0];
...
// Get a list of cities and count projects per theme
const getCities = async (req, res) => {
  try {
    const cities = await cityModel.find({}, { City_Name_EN: 1, _id: 0 });
    for (const city of cities) {
      const projectCounts = await countProjects(city.City_Name_EN);
      await updateCityInterventionType(city.City_Name_EN, projectCounts);
    }
    res.send(cities);
  } catch (error) {
    res.status(500).send({ message: 'Error processing cities', error: error.toString() });
  }
};
