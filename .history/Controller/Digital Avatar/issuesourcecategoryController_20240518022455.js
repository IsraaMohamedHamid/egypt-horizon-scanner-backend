...
import { issueSourceCategorySchema,
    IssueSourceCategoryModel} from '../../Model/Digital Avatar/issueSourceCategory.js';

// Create a new IssueSourceCategory
export async function createIssueSourceCategory(req, res) {
    const newIssueSourceCategory = new issueSourceCategorySchema(req.body);
    try {
...
export async function getAllIssueSourceCategorys(req, res) {
    try {
        const issuesourcecategorys = await IssueSourceCategoryModel.find();
...
export async function getIssueSourceCategoryById(req, res) {
    try {
        const issuesourcecategory = await IssueSourceCategoryModel.findById(req.params.id);
...
export async function updateIssueSourceCategory(req, res) {
    try {
        const updatedIssueSourceCategory = await IssueSourceCategoryModel.findByIdAndUpdate(req.params.id, req.body, { new: true });
...
export async function deleteIssueSourceCategory(req, res) {
    try {
        await IssueSourceCategoryModel.findByIdAndDelete(req.params.id);
