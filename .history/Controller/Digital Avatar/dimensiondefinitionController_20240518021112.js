...
import DimensionDefinition, { find, findById, findByIdAndUpdate, findByIdAndDelete } from '../../Model/Digital Avatar/dimensionDefinition.js';
...
export async function getAllDimensionDefinitions(req, res) {
    try {
        const dimensiondefinitions = await find();
...
export async function getDimensionDefinitionById(req, res) {
    try {
        const dimensiondefinition = await findById(req.params.id);
...
export async function updateDimensionDefinition(req, res) {
    try {
        const updatedDimensionDefinition = await findByIdAndUpdate(req.params.id, req.body, { new: true });
