...
import { attendantsSchema,
    AttendantshModel } from '../../Model/Service Offer/attendants_model.js';
...
export async function getAttendantById(req, res) {
    try {
        ...
        if (!attendants) return res.status(404).json({ message: 'Attendant not found' });
        res.json(attendants);
    } catch (error) {
        res.status(500).json({ message: error.message });
    }
